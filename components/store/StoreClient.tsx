...
export function StoreClient() {
    ...
    return (
      <div className="mx-auto w-full max-w-7xl px-4 py-7 sm:px-6 sm:py-12 lg:px-8">
        <section className="max-w-3xl">
          <span className="section-kicker">{t("Bubbleit Store")}</span>
          <h1 className="section-title mt-4">{t("Professional car care, delivered")}</h1>
          <p className="section-copy mt-4">
            {t("The same practical tools and towels trusted by Bubbleit detailers, ready for delivery across Qatar.")}
          </p>
        </section>
        <section
        ...
        >
          <h2 className="text-xl font-bold text-[color:var(--navy)]">
            {catalogState === "loading" ? t("Loading the store…") : t("The store is temporarily unavailable")}
          </h2>
          <p className="mx-auto mt-2 max-w-lg text-sm text-[color:var(--muted-foreground)]">
            {catalogState === "loading"
              ? t("We’re checking current products, prices, and stock.")
              : t("We couldn’t verify current products or stock. No offline products have been added to your cart.")}
          </p>
          {catalogState === "error" && (
            <button
              type="button"
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-7 pb-32 sm:px-6 sm:py-12 sm:pb-36 lg:px-8">
      ...
      <section className="mt-9 border-y border-[color:var(--border)] py-4" aria-label={t("Find products")}>
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <label className="relative block md:w-80">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} className="wizard-input min-h-12 ps-11" placeholder={t("Search car care products")} />
          </label>
          <div className="flex gap-2 overflow-x-auto pb-1" role="group" aria-label={t("Product categories")}>
            {categories.map((item) => <button key={item} type="button" onClick={() => setCategory(item)} className={clsx("min-h-11 shrink-0 rounded-full border px-4 text-sm font-semibold transition-colors", category === item ? "border-[color:var(--navy)] bg-[color:var(--navy)] text-white" : "border-[color:var(--border)] bg-white hover:border-[color:var(--blue)]")}>{t(item)}</button>)}
          </div>
        </div>
      </section>

      <div className="mt-8 flex items-center justify-between"><h2 className="text-xl font-bold">{t("Shop all products")}</h2><span className="text-sm text-[color:var(--muted-foreground)]">{visibleProducts.length} {t("products")}</span></div>
            ...
            <div className="flex flex-1 flex-col p-1.5 pt-4 sm:p-2 sm:pt-5">
              <div className="flex flex-col gap-2">
                ...
                <p className="shrink-0 text-base font-extrabold text-[color:var(--navy)]">
                  {formatStorePrice(product.price, lang)}
                </p>
              </div>
              <p className="mt-2 hidden min-h-12 text-sm leading-6 text-[color:var(--muted-foreground)] sm:line-clamp-2">
                {localized(lang, product.description ?? "", product.description_ar)}
              </p>
              <p className="mt-4 text-xs font-semibold text-[color:var(--muted-foreground)] sm:mt-3">
                {available > 0 ? t("Available") : t("Out of stock")}
              </p>
              {inCart > 0 ? <div className="mt-auto flex min-h-12 items-center justify-between rounded-full border border-[color:var(--border)] bg-white p-1 pt-1"><button type="button" className="grid h-10 w-10 place-items-center rounded-full hover:bg-slate-100" onClick={() => { const next = {...cart}; if (inCart === 1) delete next[id]; else next[id] = inCart - 1; updateCart(next); }} aria-label={`${t("Remove one")} ${localized(lang, product.name, product.name_ar)}`}>−</button><span className="text-xs font-bold sm:text-sm">{inCart} {t("in cart")}</span><button type="button" className="grid h-10 w-10 place-items-center rounded-full hover:bg-slate-100 disabled:opacity-30" disabled={available <= 0} onClick={() => addProduct(product)} aria-label={`${t("Add one")} ${localized(lang, product.name, product.name_ar)}`}>+</button></div> : <button
                type="button"
                className={clsx(
      ...
      {visibleProducts.length === 0 && <div className="py-16 text-center"><h2 className="text-xl font-bold">{t("No products found")}</h2><p className="mt-2 text-sm text-[color:var(--muted-foreground)]">{t("Try a different search or category.")}</p><button type="button" className="secondary-button mt-5" onClick={() => { setQuery(""); setCategory("All"); }}>{t("Clear filters")}</button></div>}

      {cartCount > 0 && <aside className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-3 shadow-[0_-12px_36px_rgba(38,34,98,0.12)] backdrop-blur-xl" aria-label={t("Cart actions")}>
        <div className="mx-auto grid max-w-xl grid-cols-2 gap-2 sm:gap-3">
          <button type="button" className="secondary-button relative min-h-14 px-3 text-sm sm:text-base" aria-haspopup="dialog" aria-expanded={cartOpen} onClick={openCart}>
            ...
            {t("View cart")} <span className="ms-1 text-[color:var(--muted-foreground)]">({cartCount})</span>
          </button>
          <Link href="/store/checkout" className="primary-button min-h-14 px-3 text-sm sm:text-base">{t("Checkout")} <span className="ms-1 hidden sm:inline">· {formatStorePrice(subtotal, lang)}</span><span className="ms-2 rtl:rotate-180" aria-hidden="true">→</span></Link>
        </div>
      </aside>}
      ...
      <div className={clsx("cart-backdrop fixed inset-0 z-50 bg-slate-950/35 p-3 sm:p-6", cartOpen && "is-open")} role="presentation" aria-hidden={!cartOpen} inert={!cartOpen} onMouseDown={(event) => { if (event.target === event.currentTarget) closeCart(); }}>
        <section role="dialog" aria-modal="true" aria-labelledby="cart-title" className={clsx("cart-dialog absolute inset-x-3 bottom-[calc(0.75rem+env(safe-area-inset-bottom))] mx-auto flex max-h-[min(78dvh,680px)] max-w-md flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl sm:inset-x-auto sm:end-6 sm:bottom-6 sm:w-[26rem]", cartOpen && "is-open")}>
          <header className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            <div><h2 id="cart-title" className="text-xl font-bold text-[color:var(--navy)]">{t("Your cart")}</h2><p className="mt-0.5 text-sm text-[color:var(--muted-foreground)]">{cartCount} {t(cartCount === 1 ? "item" : "items")}</p></div>
            <button type="button" className="grid h-11 w-11 cursor-pointer place-items-center rounded-full border border-slate-200 text-xl text-[color:var(--navy)] transition-colors hover:bg-slate-100 focus-visible:outline-2 focus-visible:outline-[color:var(--blue)]" onClick={closeCart} aria-label={t("Close cart")}>×</button>
          </header>

          {cartItems.length === 0 ? <div className="grid min-h-64 place-items-center px-6 text-center"><div><span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-slate-100 text-[color:var(--navy)]"><svg viewBox="0 0 24 24" fill="none" className="h-6 w-6" aria-hidden="true"><path d="M3.5 4.5h2l1.4 10.1a2 2 0 0 0 2 1.7h7.8a2 2 0 0 0 1.9-1.4L21 7.5H6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg></span><h3 className="mt-4 text-lg font-bold">{t("Your cart is empty")}</h3><p className="mt-1 text-sm text-[color:var(--muted-foreground)]">{t("Add a product to start your order.")}</p><button type="button" className="secondary-button mt-5" onClick={closeCart}>{t("Continue shopping")}</button></div></div> : <>
            <div className="overscroll-contain overflow-y-auto px-5 py-2">
              {cartItems.map(({ product, quantity }) => <article key={product.id} className="flex gap-3 border-b border-slate-100 py-4 last:border-0">
                ...
                <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><h3 className="text-sm font-bold leading-snug text-[color:var(--navy)]">{localized(lang, product.name, product.name_ar)}</h3><span className="shrink-0 text-sm font-extrabold">{formatStorePrice(product.price * quantity, lang)}</span></div><div className="mt-3 flex items-center gap-2"><button type="button" className="grid h-11 w-11 cursor-pointer place-items-center rounded-full border border-slate-200 text-lg font-bold hover:bg-slate-100" onClick={() => changeQuantity(product, quantity - 1)} aria-label={`${t("Remove one")} ${localized(lang, product.name, product.name_ar)}`}>−</button><span className="min-w-6 text-center text-sm font-bold" aria-label={`${quantity} ${t("in cart")}`}>{quantity}</span><button type="button" className="grid h-11 w-11 cursor-pointer place-items-center rounded-full border border-slate-200 text-lg font-bold hover:bg-slate-100 disabled:opacity-30" disabled={quantity >= availableFor(product)} onClick={() => changeQuantity(product, quantity + 1)} aria-label={`${t("Add one")} ${localized(lang, product.name, product.name_ar)}`}>+</button></div></div>
              </article>)}
            </div>
            <footer className="border-t border-slate-200 bg-slate-50 px-5 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-4 sm:pb-5"><div className="flex items-end justify-between"><span className="text-sm font-semibold text-[color:var(--muted-foreground)]">{t("Subtotal")}</span><span className="text-2xl font-extrabold text-[color:var(--navy)]">{formatStorePrice(subtotal, lang)}</span></div><p className="mt-1 text-xs text-[color:var(--muted-foreground)]">{t("Delivery details are confirmed at checkout.")}</p><Link href="/store/checkout" className="primary-button mt-4 min-h-14 w-full text-base" onClick={() => setCartOpen(false)}>{t("Checkout securely")} <span aria-hidden="true" className="ms-2 rtl:rotate-180">→</span></Link><button type="button" className="mt-2 min-h-11 w-full cursor-pointer text-sm font-semibold text-[color:var(--muted-foreground)] hover:text-[color:var(--navy)]" onClick={closeCart}>{t("Continue shopping")}</button></footer>
          </>}
        </section>
      </div>
    </div>
